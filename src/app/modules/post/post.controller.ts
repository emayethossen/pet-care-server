...
export const getSinglePetStoryController = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const getPetStoryByCategoryController = async (
  req: Request,
  res: Response,
) => {
  try {
...
// Upvote a story
export const upvotePetStoryController = async (
  req: Request,
  res: Response,
) => {};

// Downvote a story
export const downvotePetStoryController = async (
  req: Request,
  res: Response,
) => {};

// Get pet story with comments
export const getPetStoryWithCommentsController = async (
  req: Request,
  res: Response,
) => {
  try {
  ...
  try {
    ...
    if (!hasAccess) {
      return res
        ...
        .json({
          message:
            "Access denied. Please make payment to access premium content.",
        });
    }
...
// Fetch all content
export const getAllUserContentController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: "Fetched all user content successfully.",
        data: stories,
      });
  } catch (error: any) {
    res
      .status(500)
      .json({ message: "Failed to fetch user content.", error: error.message });
  }
};

export const deletePostByAdminController = async (
  req: Request,
  res: Response,
) => {
  const postId = req.params.id; // Assume the post ID is passed as a URL parameter

  try {
    const deletedPost = await PetStory.findByIdAndDelete(postId);
