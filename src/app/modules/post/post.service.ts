...
export const updatePetStory = async (
  id: string,
  data: Partial<IPetStory>,
): Promise<IPetStory | null> => {
  return PetStory.findByIdAndUpdate(id, data, { new: true });
};

export const deletePetStory = async (
  id: string,
  userId: string,
): Promise<IPetStory | null> => {
...
export const getPetStoryByCategory = async (
  category: string,
): Promise<IPetStory[]> => {
...
export const upvotePetStory = async (id: string): Promise<IPetStory | null> => {
  return PetStory.findByIdAndUpdate(
    id,
    { $inc: { upvotes: 1 } },
    { new: true },
  );
...
export const downvotePetStory = async (
  id: string,
): Promise<IPetStory | null> => {
  return PetStory.findByIdAndUpdate(
    id,
...
export const addCommentToPetStory = async (
  id: string,
  commentId: string,
): Promise<IPetStory | null> => {
  return PetStory.findByIdAndUpdate(
